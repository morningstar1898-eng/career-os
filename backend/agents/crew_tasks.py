...
def build_tasks(agents: dict) -> list:
    ...
    task_lesson = Task(
        description=(
            f"Today is {TODAY}. Build {NAME} a comprehensive DAILY LESSON that moves her toward "
            f"{SALARY_TARGET} Data / Analytics / Data / ML / AI Engineering roles. ROTATE the focus day "
            "to day so that over ~2 weeks it COMPREHENSIVELY covers her whole target stack: SQL, Python, "
            "Power BI (semantic model + DAX + RLS), Snowflake, dbt, Azure data engineering (Data Factory, "
            "ADLS, Synapse), Databricks (Spark, Delta Lake), machine learning, and LLM/AI engineering. "
            "Prefer the tool she is currently building in her portfolio roadmap (Snowflake/Azure first) "
            "when relevant; otherwise advance the rotation. Pick ONE focused topic for today and produce, "
            "in markdown:\n"
            "## Today's Focus (topic + why it matters for the salary target, 2 sentences)\n"
            "## Lesson Plan (numbered, timeboxed ~45-60 min agenda)\n"
            "## Step-by-Step How-To (exact hands-on steps + commented code/SQL for the topic)\n"
            ...
            "example, and the trade-off/gotcha interviewers are probing for. Make these genuinely deep, "
            "not surface-level.\n"
            "## Check Yourself (3 quick questions)\n"
            "## Free Resource (one specific link) and ## Tomorrow's Preview (one line).\n"
            "Be concrete and senior-level — she should be able to both DO the skill today AND confidently "
            "ANSWER for it in an interview."
        ),
        expected_output=(
            "A comprehensive daily lesson in markdown: timeboxed agenda, step-by-step how-to with code, "
            "practice + solution, FIVE in-depth interview Q&As on the topic, self-check, and next-day preview."
        ),
        agent=agents["tutor"],
        context=[task_scan],
    )
    ...
    task_interview = Task(
        description=(
            ...
            "**2. Technical — Engineering Stack (rotate the tool day to day)** — Ask ONE realistic "
            "technical question on a tool from her target stack, rotating across: advanced SQL (window "
            "functions, query optimization, partition pruning), Snowflake, Azure (Data Factory/Synapse), "
            "Databricks/Spark + Delta, dbt, and Python data pipelines. Prefer the tool she is currently "
            "building (Snowflake/Azure first). Give the correct answer with a clear explanation and the "
            f"trade-off an interviewer is probing for. Aim at the {SALARY_TARGET} level, not entry-level.\n\n"
            "**3. System Design / Architecture** — Pose a realistic data-engineering design prompt "
            "(e.g. 'design a pipeline to ingest daily Medicare claims into a cloud warehouse with quality "
            "checks and incremental loads', or 'how would you model this in Snowflake + dbt'). Provide a "
            "structured senior-level answer: requirements → architecture (ingest → store → transform → "
            "serve) → tool choices and WHY → how you handle scale, cost, and data quality.\n\n"
    ...
    task_brief = Task(
        description=(
            ...
            "## Today's Lesson & Interview Q&A (lesson topic + the 5 interview questions covered, with answers), "
