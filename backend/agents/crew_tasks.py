...
def build_tasks(agents: dict) -> list:
    ...
    task_apply = Task(
        description=(
            ...
            "SLOT MIX (mandatory): of the "
            f"{FULL_APPLICATIONS} roles, draft AT MOST 2 Optum/UHG internal roles (pick the "
            "best-fit ones) and AT LEAST 3 EXTERNAL roles. Among externals, prefer roles whose "
            "posting URL is on boards.greenhouse.io, job-boards.greenhouse.io, jobs.lever.co, "
            "or jobs.ashbyhq.com — those can be auto-submitted downstream; other external "
            "roles are fine for the remaining slots. (Internal roles previously filled every "
            "slot, which starved the auto-submit pipeline: it may never touch internal roles.)\n\n"
            "═══════════════════════════════════════════════════════════\n"
